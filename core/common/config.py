...
class LeakGuardConfig(BaseModel):
    ...
    max_files_limit: int = Field(default=50_000, description="Maximum files to discover per scan")
    max_ast_depth: int = Field(default=500, description="Maximum AST recursion depth to prevent stack overflow")
    max_cfg_nodes_per_func: int = Field(default=10_000, description="Maximum basic blocks per function")
